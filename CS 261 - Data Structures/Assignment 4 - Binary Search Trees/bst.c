...
struct Node *_addNode(struct Node *cur, TYPE val)
{
	...
	// if the value to be added is less than the value at the current node
	if(compare(val, cur->val) == -1)
	{
		cur->left = _addNode(cur->left, val);
	}

	// if the value to be added is greater than or equal to the value at the current node
	else
	{
...
int containsBSTree(struct BSTree *tree, TYPE val)
{
    ...
    // creates temporary tree
	struct BSTree *temp = newBSTree();
	assert(temp != NULL);

	// if the value passed in equals the value at the root of the tree
	...
	// if the value passed in is less than the value at the root of the tree
	else if(compare(val, tree->root->val) == -1)
	{
	    // assigns the left child to be the root of the temp tree
	    temp->root = tree->root->left;

	}

	// if the value passed in is greater than the value at the root of the tree
	else if(compare(val, tree->root->val) == 1)
	{
	    // assigns the right child to be the root of the temp tree
	    temp->root = tree->root->right;

	}

	// if the end of the tree is reached without finding the value
	if(temp->root == NULL)
	{
	  clearBSTree(temp);
	  deleteBSTree(temp);
	...
	// recursively call the contains function with the temp tree
	return containsBSTree(temp, val);
}
...
TYPE _leftMost(struct Node *cur)
{
	...
	// if the left child is null, we have reached the leftmost node
	if (cur->left == NULL)
    {
        return cur->val;
    }

    // recursively call the leftmost function with the next node to the left
...
struct Node *_removeLeftMost(struct Node *cur)
{
	...
	// if the left child is not null, there are more nodes to the left
	if(cur->left != NULL)
	{
		// recursively call the removeleftmost function with the next node to the left
		cur->left = _removeLeftMost(cur->left);
		return cur;
	}

	// once the leftmost node is reached, assign its right child to temp
	temp =  cur->right;
	free(cur);
	return temp;
}
...
struct Node *_removeNode(struct Node *cur, TYPE val)
{
	...
	// if the value to be removed is equal to the value at the current node
	if(compare(val,cur->val)==0)
	{
		// if the current node has a right child
		if(cur->right != NULL)
		{
		    // assign value at the current node to the leftmost child of its right subtree
	...
	// if the value to be removed is less than the value at the current node
	else if(compare(val, cur->val)==-1)
	{
		// recursively call the removenode function with the next node to the left
		cur->left = _removeNode(cur->left,val);
	}

	// if the value to be removed is greater than the value at the current node
	else
	{
	    // recursively call the removenode function with the next node to the right
...
Main function for testing different function of the Assignment#4.
