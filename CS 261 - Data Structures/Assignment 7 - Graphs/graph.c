...
int dfsIterative(Graph* graph, Vertex* source, Vertex* destination)
{
	...
	while(!(dequeIsEmpty(stack)))
	{
        ...
        // look at neighbors and check if any need to be visited
		int i;
        for(i = 0; i < temp->numNeighbors; i++)
        {
            if(!(temp->neighbors[i]->isVisited))
                dequePushFront(stack, temp->neighbors[i]);
        }
	}
