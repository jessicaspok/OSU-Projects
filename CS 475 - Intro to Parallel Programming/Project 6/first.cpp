...
main( int argc, char *argv[ ] )
{
    ...
    if( status != CL_SUCCESS )
      fprintf( stderr, "clSetKernelArg failed (4)\n" );
		...
		/*for( int i = 0; i < NUM_ELEMENTS; i++ )
		{
			double expected = hA[i] * hB[i] + hD[i];
			if( fabs( hC[i] - expected ) > TOL )
			{
				fprintf( stderr, "%4d: %13.6f * %13.6f + %13.6f wrongly produced %13.6f instead of %13.6f (%13.8f)\n",
					i, hA[i], hB[i], hD[i], hC[i], expected, fabs(hC[i]-expected) );
				fprintf( stderr, "%4d:    0x%08x *    0x%08x +    0x%08x wrongly produced wrongly produced    0x%08x instead of    0x%08x\n",
					i, LookAtTheBits(hA[i]), LookAtTheBits(hB[i]), LookAtTheBits(hD[i]), LookAtTheBits(hC[i]), LookAtTheBits(expected) );
			}
		}*/

		double megaMults = (double)NUM_ELEMENTS/(time1-time0)/1000000.0;

		if (megaMults > maxMegaMults)
			maxMegaMults = megaMults;
		sumMegaMults += megaMults;
	}
	fprintf( stderr, "\nNum Elements: %8d\tLocal Size: %4d\tNum Work Groups: %10d\nAvg Performance:  %10.3lf MegaMults/Sec\nPeak Performance: %10.3lf MegaMults/Sec\n",
		NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, sumMegaMults / 15, maxMegaMults );
		
	fprintf(datafile, "%d, %d, %d, %f\n", NUM_ELEMENTS, LOCAL_SIZE, NUM_WORK_GROUPS, maxMegaMults);
