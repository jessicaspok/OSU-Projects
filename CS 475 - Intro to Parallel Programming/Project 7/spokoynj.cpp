...
const char *WINDOWTITLE = { "OpenCL/OpenGL Particle System -- Jessica Spokoyny" };
...
const char *CL_FILE_NAME   = { "spokoynj.cl" };
...
Display( )
{
	...
	if( ShowPerformance )
	{
		...
		curP = (float)(NUM_PARTICLES / ElapsedTime / 1000000.);
		
		fprintf(stderr, "%6.4f\n", curP);
		sprintf(str, "Frame %8d -- %6.1f MegaParticles/Sec", Frame, curP);
...
InitCL( )
{
	...
	{
		Platform = platforms[numPlatforms - 1 - i];
		// get the device id:
...
{
        char filename[128];
        sprintf( filename, "spokoynj.%03d.particles", Frame ); // use your engr login instead
        ...
        glBindBuffer( GL_ARRAY_BUFFER, hPobj );
        struct xyzw *points = (struct xyzw *) glMapBuffer( GL_ARRAY_BUFFER, GL_READ_ONLY );
        for( int i = 0; i < NUM_PARTICLES; i++ )
        {
        ...
        glBindBuffer( GL_ARRAY_BUFFER, hCobj );
        struct rgba *colors = (struct rgba *) glMapBuffer( GL_ARRAY_BUFFER, GL_READ_ONLY );
        for( int i = 0; i < NUM_PARTICLES; i++ )
        {
